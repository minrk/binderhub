...
def test_emit_event():
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        el.emit("test/test", 1, {"something": "blah",})
        handler.flush()
...
def test_emit_event_badschema():
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        with pytest.raises(jsonschema.ValidationError):
            el.emit("test/test", 1, {"something": "blah", "status": "not-in-enum"})
...
async def test_provider_completeness(app):
    ...
    """
    repo_providers = {rp.name.default_value for rp in app.repo_providers.values()}

    launch_schema_path = os.path.join(
        os.path.dirname(__file__), "..", "event-schemas", "launch.json"
    )
