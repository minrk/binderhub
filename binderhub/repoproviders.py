...
class ZenodoProvider(RepoProvider):
    ...
    def get_resolved_ref(self):
        ...
        req = HTTPRequest(
            "https://doi.org/{}".format(self.spec), user_agent="BinderHub"
        )
...
class DataverseProvider(RepoProvider):
    ...
    def get_resolved_ref(self):
        ...
        self.resolved_spec = "{authority}/{identifier}".format(
            authority=resp["data"]["authority"], identifier=resp["data"]["identifier"],
        )
...
class HydroshareProvider(RepoProvider):
    ...
    def get_resolved_ref(self):
        ...
        req = HTTPRequest(
            "https://www.hydroshare.org/hsapi/resource/{}/scimeta/elements".format(
                self.resource_id
            ),
...
class GitRepoProvider(RepoProvider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.unresolved_ref:
            raise ValueError(
                "`unresolved_ref` must be specified as a query parameter for the basic git provider"
            )
    ...
    def get_resolved_ref(self):
        ...
        try:
            ...
            if result.returncode:
                raise RuntimeError(
                    "Unable to run git ls-remote to get the `resolved_ref`: {}".format(
                        result.stderr
                    )
                )
...
class GitHubRepoProvider(RepoProvider):
            ...
            if e.code == 304:
                ...
                self.log.error(
                    "GitHub Rate limit ({limit}) exceeded. Reset in {delta}.".format(
                        limit=rate_limit, delta=timedelta(seconds=reset_seconds),
                    )
                )
    ...
    def get_resolved_ref(self):
        ...
        self.cache.set(
            api_url, {"etag": resp.headers.get("ETag"), "sha": self.resolved_ref,},
        )
        return self.resolved_ref
