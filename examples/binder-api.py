...
def build_binder(
    repo, ref="master", filepath=None, *, binder_url="https://mybinder.org"
):
    """Launch a binder
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filepath", type=str, help="The file to open, if any.")
    parser.add_argument(
        "--binder",
    ...
    for evt in build_binder(
        opts.repo, ref=opts.ref, filepath=opts.filepath, binder_url=opts.binder
    ):
        if "message" in evt:
            print(
                "[{phase}] {message}".format(
                    phase=evt.get("phase", ""), message=evt["message"].rstrip(),
                )
            )
